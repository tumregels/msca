...
class Config(object):
    ...
    LIB_DIR = op.abspath(op.join(CWD, os.pardir, os.pardir, os.pardir, 'libraries/l_endian'))
...
# start calculation similar to $ dragon_exe < dragon_input.x2m | tee dragon_output.result
# https://stackoverflow.com/a/28319191
# https://stackoverflow.com/a/163556
with \
    Popen(Config.DRAGON_EXE, stdout=PIPE, 
          stdin=PIPE, bufsize=1, universal_newlines=True, cwd=Config.OUTPUT_DIR, encoding='utf8') as p, \
    open(Config.DRAGON_INPUT_FILE) as inp, \
    open(Config.DRAGON_OUTPUT_FILE, 'w') as out:
