...
dpath = '/../../Dragon/2L/ASSBLY_C/output_2020-09-04_15-57-35/';
spath = '/../../Serpent/ASSBLY_C/output_2020-08-31_18-19-22/';

s(1).drag_filename = fullfile(filedir, dpath, 'ASSBLY_CASEC_R_BCOND2.txt');
s(1).serp_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_det0.m');
s(1).serp_dep_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_dep.m');
s(1).results_fission = fullfile(filedir, dpath, 'process_cfm_first.result');
s(1).results_capture = fullfile(filedir, dpath, 'process_ccm_first.result');

s(2).drag_filename = fullfile(filedir, dpath, 'ASSBLY_CASEC_R_BCOND2_PEAK.txt');
s(2).serp_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_det62.m');
s(2).serp_dep_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_dep.m');
s(2).results_fission = fullfile(filedir, dpath, 'process_cfm_peak.result');
s(2).results_capture = fullfile(filedir, dpath, 'process_ccm_peak.result');

s(3).drag_filename = fullfile(filedir, dpath, 'ASSBLY_CASEC_R_BCOND2_LAST.txt');
s(3).serp_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_det107.m');
s(3).serp_dep_filename = fullfile(filedir, spath, 'ASSBLY_CASEC_mc_dep.m');
s(3).results_fission = fullfile(filedir, dpath, 'process_cfm_last.result');
s(3).results_capture = fullfile(filedir, dpath, 'process_ccm_last.result');

gd_cells = {'C0603', 'C0707'};
...
for i = 1:numel(s)
    ...
    mat_filename = fullfile(filedir, '..', '..', 'data', sprintf('parse_dragon_c_2l_%02d.mat', i));
